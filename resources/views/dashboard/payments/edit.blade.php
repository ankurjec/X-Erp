         	...
         	<div class="col-xs-12 col-sm-12 col-md-12">
		        <div class="form-group">
		            ...
		            <select id="expense_ids" name="expense_ids[]" class="form-control" multiple="multiple">
		                ...
		                <option value="{{$expense->id}}" @if(in_array( $expense->id ,$payment->expense_ids )) selected="delected" @endif>#{{$expense->id}} (Rs.{{$expense->amount}})</option>
		                @endforeach
		            </select>
		        </div>
		    </div>
		        ...
		        <div class="form-group">
		            ...
		            <select name="user_id" class="form-control user_id_select select2">
		                ...
		                @foreach($users as $user)
    		                @if($user->id > 1)
    		                <option value="{{$user->id}}" @if($payment->user_id == $user->id) selected="selected" @endif>{{$user->name}}</option>
    		                @endif
		                @endforeach
		            </select>
		        </div>
		    ...
		    <div class="col-xs-12 col-sm-12 col-md-12 vendor_id_select_box" style="display:none">
		        <div class="form-group">
		            ...
		            <select name="vendor_id" class="form-control vendor_id_select select2">
		                ...
		                <option value="{{$vendor->id}}" @if($payment->vendor_id == $vendor->id) selected="selected" @endif>{{$vendor->name}}</option>
		                @endforeach
		            </select>
		        </div>
		    </div>
		    <div class="col-xs-12 col-sm-12 col-md-12 customer_id_select_box" style="display:none">
		        <div class="form-group">
		            ...
		            <select name="customer_id" class="form-control customer_id_select select2">
		                ...
		                <option value="{{$customer->id}}" @if($payment->customer_id == $customer->id) selected="selected" @endif>{{$customer->name}}</option>
		                @endforeach
		            </select>
		        </div>
		    </div>
        <div class="col-xs-12 col-sm-12 col-md-12">
          <div class="form-group">
              ...
              <select name="final_beneficiary_id" class="form-control select2">
                  <option value="" disabled selected="selected">Select</option>
                  @foreach($vendors as $vendor)
                  <option value="{{$vendor->id}}" @if($payment->final_beneficiary_id == $vendor->id) selected="selected" @endif>{{$vendor->name}}</option>
                  @endforeach
              </select>
          </div>
      </div>
...
<script type="text/javascript">
    $(document).ready(function(){
       $(document).on('change','.entity_type_select',function(e){
           PaidEntityVisibility();
       });
       
       PaidEntityVisibility();
       
       function PaidEntityVisibility() {
