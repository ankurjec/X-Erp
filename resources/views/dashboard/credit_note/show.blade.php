...
<div class="c-subheader px-3">
          ...
          <ol class="breadcrumb border-0 m-0">
            ...
            <li class="breadcrumb-item active">Show credit Notes</li>
            <!-- Breadcrumb Menu-->
          </ol>
</div>
@endsection
    ...
    <div class="expense_row row">
		        ...
		        <div class="form-group">
		            ...
		            <select name="user_id" class="form-control user_id_select" disabled="true">
		                ...
		                @foreach($users as $user)
    		                @if($user->id > 1)
    		                <option value="{{$user->id}}" @if($credit_note->user_id == $user->id) selected="selected" @endif>{{$user->name}}</option>
    		                @endif
		                @endforeach
		            </select>
		        </div>
		    ...
		    <div class="col-xs-2 col-sm-2 col-md-2">
		        <div class="form-group">
		            ...
		            <input type="text" name="amount" class="form-control" placeholder="Amount" value="{{moneyFormatIndia($credit_note->amount)}}" readonly>
		        </div>
		    </div>
		    ...
		    <div class="col-xs-4 col-sm-4 col-md-4">
		        <div class="form-group">
		            ...
		            <textarea class="form-control" name="details" placeholder="Detail" readonly>{!!$credit_note->details!!}</textarea>
		        </div>
		    </div>
		        ...
		        <div class="form-group">
		            ...
		            <select name="order_id" class="form-control" disabled="true">
		                ...
		                @foreach($orders as $order)
    		                <option value="{{$order->id}}" @if($credit_note->order_id == $order->id) selected="selected" @endif>#{{$order->id}}-{{substr($order->name,0,20)}}</option>
		                @endforeach
		            </select>
		        </div>
		    </div>
				...
				<div class="form-group">
					...
					<select name="loan_id" class="form-control" disabled="true">
						...
						@foreach ($loans as $loan)
							<option value="{{ $loan->id }}" @if($credit_note->loan_id == $loan->id) selected="selected" @endif>
		    ...
		    <div class="col-xs-4 col-sm-4 col-md-4">
		        <div class="form-check">
                  <input disabled="true" name="paid_flag" class="form-check-input" type="checkbox" value="1" id="paid_flag" @if($credit_note->paid_flag) checked="checked" @endif>
